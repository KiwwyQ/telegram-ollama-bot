...
(async), `httpx` for Ollama Cloud calls, and `aiosqlite` for persistent storage.
...
| **GIFs** | The model can send GIFs (Klipy, free, no key). |
...
| **Robustness** | Restart-resilient SQLite storage, graceful shutdown, structured logging with secret redaction. |
